...
export default function InputPassword({ labelText, inputId }) {
	...
	const validatePassword = useCallback((value) => {
		...
		// регулярка с учётом спец.символов
		// if (!/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+\-=\\[\]{}|;:'",.<>?\\/~])[a-zA-Z\d!@#$%^&*()_+\-=[\]{}|;:'",.<>?\\/~]{6,25}$/.test(value)) {
		// 	return 'Пароль должен содержать хотя бы одну заглавную букву, одну цифру и один спец.символ';
		// }
